...
const register = async (req, res) => {
  try {
    ...
    const rows = await new Promise((resolve, reject) => {
      db.query(
        "SELECT user_id FROM registered_data WHERE email = ?",
        [normalizedEmail],
        (err, result) => {
          if (err) reject(err);
          else resolve(result);
    ...
    await new Promise((resolve, reject) => {
      db.query(
        "INSERT INTO registered_data (user_name, email, password) VALUES (?, ?, ?)",
...
const user_data = async (req, res) => {
  ...
  db.query(sql, [name, email, mobile_no, subject, message], (err, results) => {
    ...
    res.status(201).json({
      success: true,
      message:
        "Thank you for submitting the form! Our team will connect with you as soon as possible.",
    });
